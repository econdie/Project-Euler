def RowProducts(matrix):
	tempMax = 0
	for m in range(0, len(matrix)):
		for n in range(0, len(matrix[m])-3):
			currentProduct = int(matrix[m][n])*int(matrix[m][n+1])*int(matrix[m][n+2])*int(matrix[m][n+3])
			if currentProduct > tempMax:
				tempMax = currentProduct

	return tempMax

def ColumnProducts(matrix):
	tempMax = 0
	for m in range(0, len(matrix)-3):
		for n in range(0, len(matrix[m])):
			currentProduct = int(matrix[m][n])*int(matrix[m+1][n])*int(matrix[m+2][n])*int(matrix[m+3][n])
			if currentProduct > tempMax:
				tempMax = currentProduct

	return tempMax

def DiagonalProducts(matrix):
	tempMax = 0
	for m in range(0, len(matrix)-3):
		for n in range(0, len(matrix[m])-3):
			currentProduct = int(matrix[m][n])*int(matrix[m+1][n+1])*int(matrix[m+2][n+2])*int(matrix[m+3][n+3])
			if currentProduct > tempMax:
				tempMax = currentProduct

	return tempMax

def DiagonalProducts2(matrix):
	tempMax = 0
	for m in range(3, len(matrix)):
		for n in range(0, len(matrix[m])-3):
			currentProduct = int(matrix[m][n])*int(matrix[m-1][n+1])*int(matrix[m-2][n+2])*int(matrix[m-3][n+3])
			if currentProduct > tempMax:
				tempMax = currentProduct
...
numbers = ['08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08'.split(),
...
##four different directions to check, get max in each direction
rowProduct = RowProducts(numbers)
columnProduct = ColumnProducts(numbers)
diagonalProduct = DiagonalProducts(numbers)
diagonalProduct2 = DiagonalProducts2(numbers)

##print the max of the different directions
print(max(rowProduct, columnProduct, diagonalProduct, diagonalProduct2))